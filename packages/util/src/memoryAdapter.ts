...
const localSessionIndex: Map<string, { sessionId: string; createdAt: Date }[]> = new Map()
...
class AgentCoreMemoryAdapter {
  ...
  constructor(config?: { memoryId?: string; namespace?: string }) {
    ...
    this.namespace = config?.namespace || process.env.AGENTCORE_MEMORY_NAMESPACE || '{actorId}'

    // Use local fallback if no memory ID is configured
    this.useLocalFallback = !this.memoryId

    if (this.useLocalFallback) {
      console.log('[MemoryAdapter] No AgentCore Memory ID configured, using local fallback')
    }
  }
   ...
   */
  async logConversationEvent(actorId: string, sessionId: string, role: MessageRole, content: string): Promise<void> {
   ...
   */
  async recall(actorId: string, query: string, topK: number = 5): Promise<string[]> {
    if (this.useLocalFallback) {
  ...
  ): Promise<{ eventId: string; role?: string; text?: string; eventTimestamp?: Date }[]> {
