...
function MermaidBlock({ code, id }: Readonly<MermaidBlockProps>) {
  ...
  return <div ref={containerRef} className="[&>.mermaid]:min-h-[80px]" data-mermaid="true" />
}
...
function PreBlock({ children, ...props }: React.HTMLAttributes<HTMLPreElement> & { children?: React.ReactNode }) {
  ...
  if (React.isValidElement(child) && (child.props as { 'data-mermaid'?: string })['data-mermaid'] === 'true') {
    return <>{child}</>
  }
