...
const statusConfig: Record<TimelineEventStatus, { icon: React.ElementType; color: string; label: string }> = {
...
const TimelineEvent = React.forwardRef<HTMLDivElement, TimelineEventProps>(
  ({ toolName, status, input, output, startTime, endTime, isExpanded = false, onToggle }, ref) => {
    ...
    return (
      <div ref={ref} className="relative pl-6 pb-4">
        ...
        {/* Status icon */}
        <div
          className={cn('absolute left-0 top-1 flex items-center justify-center w-4 h-4 bg-background', config.color)}
        >
        ...
        {/* Event content */}
        <div
          className={cn('rounded-lg border bg-card p-3', onToggle && 'cursor-pointer hover:bg-accent/50')}
          onClick={onToggle}
        >
          <div className="flex items-center justify-between gap-2">
            ...
            <Badge variant={status === 'failed' ? 'destructive' : 'secondary'}>{config.label}</Badge>
          </div>
