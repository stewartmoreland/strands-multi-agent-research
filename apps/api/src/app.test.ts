...
beforeEach(() => {
  ...
  vi.mocked(listFoundationModels).mockResolvedValue([{ modelId: 'model-1', modelName: 'Test Model' }])
})
...
describe('GET /sessions', () => {
  ...
  it('returns 401 for invalid token', async () => {
    const res = await request(app).get('/sessions').set('Authorization', 'Bearer invalid.jwt.here')
    expect(res.status).toBe(401)
...
describe('GET /sessions/:sessionId/events', () => {
  ...
  it('returns events with valid token', async () => {
    ...
    const res = await request(app).get('/sessions/s1/events').set('Authorization', `Bearer ${token}`)
    expect(res.status).toBe(200)
