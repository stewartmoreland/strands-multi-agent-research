...
import { getActorIdFromAuth, normalizeInvocationsBody, parseBody, sendEvent } from './serverHelpers.js'
...
async function handleInvocations(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const raw = await parseBody<InvocationRequest | { input?: InvocationRequest }>(req)
  ...
  if (wantsSSE) {
    ...
    try {
      ...
      const sessionContext = propagation.setBaggage(context.active(), sessionBaggage)
 ...
 */
async function handleSessionEvents(req: IncomingMessage, res: ServerResponse, sessionId: string): Promise<void> {
  const actorId = getActorIdFromAuth(req)
  if (!actorId) {
    res.writeHead(401, {
      'Content-Type': 'application/json',
...
async function requestHandler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    } else if (pathname.startsWith('/sessions/') && pathname.endsWith('/events') && method === 'GET') {
