...
import { createAnalysisTool, createResearchTool, createWritingTool, getDefaultModelId } from './specialists/index.js'
...
function getOrchestratorAgent(modelId: string): Agent {
  ...
  if (!agent) {
    agent = new Agent({
      ...
      tools: [createResearchTool(modelId), createAnalysisTool(modelId), createWritingTool(modelId)],
...
export const orchestrator = {
   ...
   */
  async *stream(prompt: string, context: OrchestratorContext): AsyncGenerator<UiEvent> {
    ...
    if (sessionId && userId) {
      try {
        await memoryAdapter.logConversationEvent(userId, sessionId, 'user', prompt)
      } catch (error) {
        logger.warn('Failed to log user message to memory', {
    ...
    const fullPrompt = recallContext ? `${recallContext}\n\nUser question: ${prompt}` : prompt
    ...
    if (sessionId && userId && fullResponse) {
      try {
        await memoryAdapter.logConversationEvent(userId, sessionId, 'assistant', fullResponse)
      } catch (error) {
        logger.warn('Failed to log assistant message to memory', {
          error: error instanceof Error ? error.message : String(error),
        })
      }
    }
...
function mapStrandsEventToUiEvent(event: unknown): UiEvent | null {
  ...
  if (evt.type === 'beforeToolCallEvent') {
    ...
    const toolName = (toolUse?.name as string) ?? (evt.toolName as string) ?? 'unknown'
  ...
  if (evt.type === 'afterToolCallEvent') {
    ...
    const output = result != null ? toolResultBlockToString(result) : ((evt.output as string) ?? '')
  ...
  // Errors
  if (evt.type === 'error') {
    const message = (evt.message as string) ?? (evt.error as string) ?? 'Unknown error'
