...
export function MfaSetupPage() {
  ...
  const { setupTOTP, verifyTOTP, setMfaPreference, isAuthenticated, user } =
    useAuth();
  ...
  const email =
    user?.email || (location.state as { email?: string })?.email || "user";
  ...
  useEffect(() => {
    const fetchSecret = async () => {
      try {
        ...
        const message =
          err instanceof Error ? err.message : "Failed to set up MFA";
        setError(message);
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        message.includes("EnableSoftwareTokenMFAException") ||
