 ...
 */
export function useSessionEvents(sessionId: string | null, getAuthToken?: () => Promise<string | null>) {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      const list: Message[] = (data.events ?? [])
        ...
        .sort((a, b) => new Date(a.eventTimestamp ?? 0).getTime() - new Date(b.eventTimestamp ?? 0).getTime())
        .map((e, i) => ({
          id: e.eventId || `evt-${i}`,
          role: e.role,
          content: e.text.trim(),
          timestamp: e.eventTimestamp ? new Date(e.eventTimestamp).getTime() : Date.now(),
        }))
      setMessages(list)
    } catch (err) {
      const message =
